...
In order to determine the final position sizes, a Markowitz-type optimization problem with price impact is solved. In the classical formulation of the problem, we have an agent maximizing expected utility of CARA type ($u(x) = 1 - e^{-\lambda x}$, where $\lambda$ is a risk-aversion parameter). If $x$ is the terminal wealth of a portfolio whose initial value is $p$, and which is composed of $d$ assets whose returns are normally distributed with expected value vector $\mu$ and covariance matrix $\Sigma$, then the agent chooses weights $w$ such that $w^{\top} \mu - \frac{\lambda p}{2} w^{\top} \Sigma w$ is maximized, typically with a constraint that $\sum_{i=1}^{d} w_i = 1$.
...
$$\max_w f(w) = w^{\top} \mu(w) - \frac{\lambda p}{2} w^{\top} \Sigma w$$
...
- $g_i = 2 \times \eta_{lin, i} \times \frac{\sigma_i}{V_i}$ for $i = 1, ..., d$, where $\eta_{lin, i}$ is the estimated coefficient, $\sigma_i$ and $V_i$ are respective EWMA realized volatility and EWMA volume (as defined for [Weights Optimization Model](/README.md#weights-optimization-model)) for asset $i$,

then we have:

$$f(w) = w^{\top} \left(s - p G w \right) - \frac{\lambda p}{2} w^{\top} \Sigma w = w^{\top} s - w^{\top} \left(p G + \frac{\lambda p}{2} \Sigma \right) w,$$

and:

$$\nabla f(w) = s - \left(2 p G + \lambda p \Sigma \right) w$$

Since $2 p G + \lambda p \Sigma$ is positive definite (see below), Hessian of $f$ is negative definite, and thus, $\nabla f(w) = 0$ gives a local (and in this case - global) maximum. Therefore, the solution to the optimization problem defined above is:

$$w^* = \left(2 p G + \lambda p \Sigma \right)^{-1} s$$

It can be easily shown that $2 p G + \lambda p \Sigma$ is strictly positive definite, and hence invertible. If $\Sigma$ is a proper covariance matrix (without perfect correlations), and $\lambda p$ is strictly positive (which it is by definition), then $\lambda p \Sigma$ must be positive definite. $G$ (and thus $2 p G$) is trivially positive definite, given it is a diagonal matrix with positive entries (price impact is positive for a buy and negative for a sell). Then, $w^{\top} \left(\lambda p \Sigma \right) w > 0$, $w^{\top} \left(2 p G \right) w > 0$, and $w^{\top} \left(2 p G + \lambda p \Sigma \right) w > 0$ for all $w \neq 0$.

$\Sigma$ is constructed by combining empirical correlation matrix estimated on the respective calibration period (the same as in case of PCA and ARMA) and rolling volatilities re-estimated each time-step. In the presented backtesting only EWMA volatility has been used. $\lambda$ is a parameter which may be adjusted according to needs. If changes in portfolio's value over the trading period are assumed to be insignificant, $\lambda p$ can be replaced with a single fixed parameter describing (relative) risk aversion.
...
The specific steps taken in each walk-forward are as follows. First, data for respective periods is filtered. Then, Genetic Algorithm is run on the in-sample period, and extended performance statistics for top individuals from the last generation are obtained, as described in [Basket Selection](/README.md#basket-selection). The final basket is selected based on the weighted average of performance metrics in the neighbourhood. However, in order to avoid spikes in the searched space, additional conditions or corrections may be imposed. This includes:
- (iteratively) filtering out baskets for which the performance metric exceeds the average of performance metrics in its neighbourhood, unless:
  - too many of them are removed, or
  - all of the top ones are removed;
- adding a penalty proportional to the difference between performance of the main basket and average performance in its neighbourhood.

The last option has been applied in the presented backtesting.

Once the basket is selected, ARMA predictions are generated for all considered numbers of PCs for systematic component construction and lags orders, and smoothed cumulative returns earned in individual trading are obtained for each asset and combination, as described in [Traded Assets, PCs Numbers and ARMA Orders Selection](/README.md#traded-assets-pcs-numbers-and-arma-orders-selection). For each asset, combinations are ranked according to the cumulative returns they yielded, and a specified number of top ones is selected, provided their return exceeds a specified threshold. If the threshold is exceeded only for a smaller number of combinations, additional ones, whose return exceeds a lower threshold, may be selected, but their maximal number is smaller. If no combination yields a cumulative return exceeding the lower threshold for a given asset, it will not be traded in the subsequent out-of-sample period. This way, a set of candidate combinations for each asset is chosen for the last stage of hyper-optimization, i.e., prioritized search exploring performance in joined trading, as described in [Traded Assets, PCs Numbers and ARMA Orders Selection](/README.md#traded-assets-pcs-numbers-and-arma-orders-selection). The final combination of combinations is selected based on the weighted average of performance metrics in the neighbourhood, with the same options of additional conditions or corrections as in case of basket selection. Similarly, the last option has been applied in the presented backtesting.

Once the basket, assets to be traded, number of PCs for systematic component construction and ARMA lags orders are selected, a full trading simulation on the corresponding out-of-sample period is performed with their use, as described in [Trading and Return Calculation](/README.md#trading-and-return-calculation). The earned returns, portfolio weights and portfolio values in particular time steps of the out-of-sample period are saved. The next out-of-sample period will start with the portfolio value from the end of the preceding period. In the presented backtesting the initial portfolio value has been set to $100k.
