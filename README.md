...
A basket of assets used for PCA transformation is selected via Genetic Optimization Algorithm run on an in-sample window. An individual is defined as a binary sequence of elements corresponding to each asset which can be included or excluded from the PCA transformation. A set of assets which are always included in the basket can be specified. The initial population is generated by randomly drawing 0 or 1 for each element, such that the number of assets in the basket is within a specified range. Each individual's fitness is evaluated based on performance of the strategy for a given basket and neighbouring baskets. Fitter individuals have a higher probability of propagation. Majority of the new generation is created by applying crossover and mutation on selected parents' genes. The rest is filled with randomly generated immigrants. The population size is constant over generations. The process is repeated for a specified number of iterations.
